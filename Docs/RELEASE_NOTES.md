...
- Increased Volley slate row-density progression to keep brick fields consistently populated on large/aspect-wide slates.
...
- Centralized Tunnel row-density tuning in `ModeLayoutPolicy` and increased slate vertical row boosts for Siege layouts.
...
- Further unified HUD scaling on tablet/slate/fold classes by reducing oversized control/text inflation and tightening reserved-height usage on larger slates.
- Added `VolleyModeSystem.isBallInFlight` thresholding to avoid turn-resolution stalls caused by near-zero motion jitter.
- Hardened Volley turn progression by detecting stalled non-moving balls and nudging them back into play instead of resolving turns with orphaned balls.
...
- Normalized renderer feedback envelopes in `GameRenderer` (shake clamp parity with engine profiles and smoother impact-flash accumulation/decay) to reduce cross-mode visual spikes.
...
- Fixed Tunnel gate-integrity telemetry drift by invalidating cached integrity values on any gate-brick damage (not just destruction).
